...
//                value *= 1.5F;
    ...
    public void processCommand(ICommandSender player, String[] args) {
        ...
        if ( "OPTIONS".equals( mode ) ) {
            if ( args.length == 2 && args[1].toUpperCase().equals( "BUY" )  ) {
                func_152373_a( player, this, "Buy options are: [ " +
                    String.join(", m", BUY_TRADE_ENTRIES.keySet() ) +
                    "]", new Object[0] );

            } else {
                func_152373_a( player, this, "Sell options are: [ " +
                    String.join(", ", SELL_TRADE_ENTRIES.keySet() ) +
                    "]", new Object[0] );
        ...
        switch ( mode ) {
            ...
            case "CLEAR":
                ...
                func_152373_a( player, this, "Trader: " + trader.getNPCName() + "'s trades were cleared and locked.", new Object[0] );
            ...
            case "ADD":
                if ( entry != null ) {
                    ...
                    func_152373_a( player, this, args[1].toUpperCase() + " add to " + trader.getNPCName() + "'s trades.", new Object[0] );

                } else { // entries != null
                    setTraderNpcInfo( trader, entries, false );
                    func_152373_a( player, this, args[1].toUpperCase() + " trades set for " + trader.getNPCName(), new Object[0] );
