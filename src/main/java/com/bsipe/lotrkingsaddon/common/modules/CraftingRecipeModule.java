...
public class CraftingRecipeModule extends AbstractModule {
    ...
    public void init(FMLInitializationEvent event) {
        ...
        if (config.quartz()) GameRegistry.addShapedRecipe(
            new ItemStack(Items.quartz, 4),
            new Object[] { " x ", "xvx", " x ", 'x', Blocks.sand, 'v', LOTRMod.salt });
        if (config.enderChest()) GameRegistry.addShapedRecipe(
            new ItemStack(Blocks.ender_chest),
            new Object[] { "ooo", "omo", "ooo", 'o', Blocks.obsidian, 'm', LOTRMod.mithril });
        if (config.beacon()) GameRegistry.addShapedRecipe(
            new ItemStack(Items.nether_star),
            new Object[] { "cmf", "mpm", "fmc", 'c', LOTRMod.chilling, 'm', LOTRMod.oreMithril, 'f', LOTRMod.balrogFire,
                'p', LOTRMod.pearl });
        if (config.stoneChest()) GameRegistry.addShapedRecipe(
            new ItemStack(LOTRMod.chestStone),
            new Object[] { "sss", "scs", "sss", 's', LOTRMod.scorchedStone, 'c', Blocks.chest });
        ...
        if (config.handyHardyScrolls()) {
            ...
            GameRegistry
                .addRecipe(new EnchantedBookRecipe(hardy, null, 2, ItemBlock.getItemFromBlock(Blocks.cobblestone)));
    ...
    public void addEnchantedBookCraftingRecipes() {
        RecipeSorter.register(
            MyMod.MODID + ":enchantedbookcrafting",
            EnchantedBookRecipe.class,
            SHAPELESS,
        ...
        if (config.knockback()) {
            ...
            if (config.lowTierEnchantedBooks()) {
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.knockback, 1, LOTREnchantment.knockback1, 2));
            }
        }
        ...
        if (config.fireProtection()) {
            GameRegistry
                .addRecipe(new EnchantedBookRecipe(Enchantment.fireProtection, 4, LOTREnchantment.protectFire3, 4));
            if (config.lowTierEnchantedBooks()) {
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.fireProtection, 3, LOTREnchantment.protectFire2, 4));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.fireProtection, 2, LOTREnchantment.protectFire1, 4));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.fireProtection, 1, LOTREnchantment.protectFire1, 2));
            }
        }
        if (config.projectileProtection()) {
            GameRegistry.addRecipe(
                new EnchantedBookRecipe(Enchantment.projectileProtection, 4, LOTREnchantment.protectRanged3, 4));
            if (config.lowTierEnchantedBooks()) {
                GameRegistry.addRecipe(
                    new EnchantedBookRecipe(Enchantment.projectileProtection, 3, LOTREnchantment.protectRanged2, 4));
                GameRegistry.addRecipe(
                    new EnchantedBookRecipe(Enchantment.projectileProtection, 2, LOTREnchantment.protectRanged1, 4));
                GameRegistry.addRecipe(
                    new EnchantedBookRecipe(Enchantment.projectileProtection, 1, LOTREnchantment.protectRanged1, 2));
            }
        }

        if (config.featherFalling()) {
            GameRegistry
                .addRecipe(new EnchantedBookRecipe(Enchantment.featherFalling, 4, LOTREnchantment.protectFall3, 4));
            if (config.lowTierEnchantedBooks()) {
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.featherFalling, 3, LOTREnchantment.protectFall2, 4));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.featherFalling, 2, LOTREnchantment.protectFall1, 4));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.featherFalling, 1, LOTREnchantment.protectFall1, 2));
            }
        }
        if (config.power()) {
        ...
        if (config.punch()) {
            ...
            if (config.lowTierEnchantedBooks()) {
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.punch, 1, LOTREnchantment.rangedKnockback1, 2));
            }
        }
        ...
        if (config.efficiency()) {
            ...
            if (config.lowTierEnchantedBooks()) {
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.efficiency, 4, LOTREnchantment.toolSpeed3, 5));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.efficiency, 3, LOTREnchantment.toolSpeed2, 5));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.efficiency, 2, LOTREnchantment.toolSpeed1, 5));
                GameRegistry
                    .addRecipe(new EnchantedBookRecipe(Enchantment.efficiency, 1, LOTREnchantment.toolSpeed1, 3));
            }
        }
        ...
        if (config.fireAspect()) {
            ...
            GameRegistry.addShapelessRecipe(stack, LOTRMod.balrogFire, LOTRMod.balrogFire); // these are flame of udun,
                                                                                            // just called balrog fire
                                                                                            // in the code.
        }
    }
    ...
    public void tick(EntityPlayerMP player) {
        ...
        if (config.removeReforgeCooldown() && player.openContainer.getClass()
            .equals(LOTRContainerAnvil.class)) {
        ...
        if (MyMod.SERVER_ONLY_MODE && player.openContainer.getClass()
            ...
            final ItemStack result = CraftingManager.getInstance()
                .findMatchingRecipe(crafting.craftMatrix, player.worldObj);
            ...
            if (synced.containsKey(player)) {
                ...
                if (info[0] == player.currentWindowId && info[1] == Item.getIdFromItem(result.getItem())
                    && !Items.enchanted_book.equals(result.getItem())) {
                    return;
                }
            }
